...
- 재발 방지: 검사·빌드에서 `prisma generate` 선행, pnpm 9.12.1 명시, 오래된 npm lockfile 제거 및 pnpm lockfile 단일화. [설치·검증 절차](03-infrastructure/local-dev.md).
- 별도 미해결: ESLint 설정 파일 부재. DB 마이그레이션·서비스 재시작·실발행은 수행하지 않음.

### 2026-09-09 원본 보존 카피 생성 연결 완료
- 원문/제공 이미지에서 사건·근거·반응 포인트·변경 허용 범위·미확인 정보를 구조화한 뒤 본문 생성. 텍스트 근거는 실제 원문 인용인지 코드로 검증.
- Line A·텔레그램 텍스트 재생성·성과 확산·카피 테스트 커맨드에서 본문 분석을 댓글에도 전달. 본문 재시도/복수 후보는 분석 1회 재사용. 원작자 체험 전환·없는 효과는 본문/댓글 검사에서 차단·재생성.
- RAG 조회에 SHOPPING 필터 추가. 댓글의 허구 사용경험 예시 제거. 검사 API 실패는 쇼핑 카피에서 통과 처리하지 않음.
- 검증: `npm run test:copy` 7개 통과. 실모델 일본어 원문 1건에서 '올블랙인데 이렇게 귀여운 건 또 처음 봄' 생성 및 과거 실패 문구 차단 확인. [실측 결과](09-backlog/2026-09-09-source-preservation-live.json).
- 범위: 영상 전체 분석/미디어 확보 방식/DB 스키마/실발행은 변경하지 않음. 분석은 생성 요청 안에서 공유하며 DB에 영구 저장하지 않음. 품질 선호·노출·수익 개선은 미검증. 당시 실패한 전체 typecheck는 위 후속 작업에서 복구 완료.

### 2026-09-08 쇼핑 카피 지침 변경
- 후속 수익성 감사: DB 쇼핑 PUBLISHED 13건, 72h 표본 11건 조회 중앙값 41/최고 3,359 (모두 새 지침 전). 주문·수수료 미확인. [수익성 진단·시뮬레이션](09-backlog/2026-09-08-shopping-profit-audit.md)에 개선 우선순위와 검증 계획 기록.
