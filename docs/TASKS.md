...
## 2026-09-09 개발 검증 복구
- [x] pnpm 설정·누락 의존성·Prisma Client 불일치 해소. 전체 src 타입 검사, 빌드, 카피 회귀 테스트 7개 통과. 검사·빌드 시 Client 생성 및 pnpm lockfile 단일화.
- [ ] ESLint 설정 파일 추가 및 lint 오류 정비 (타입 검사와 별도).

## 2026-09-08 쇼핑 카피 품질
- [x] 원본 장면 중심 반응형 카피 지침 반영 — 상품 강점 설명 강제 제거, 생략·미완결 허용, 원본 우선·페르소나 어투 조절, 프롬프트 내 품질 자가점검.
- [x] 원본 보존 분석 → 본문·댓글 연결, 원문 인용 검증·체험 전환 검사·SHOPPING RAG 필터 (2026-09-09). 오프라인 7개 + 실제 모델 원문 1건 검증.
- [ ] 동일 원본으로 기존/변경 지침 생성본 비교 및 사용자 품질 평가.
- [ ] 영상 핵심 장면·전개를 설명하는 입력 보강 (현재 원문 + 이미지 최대 1장).
