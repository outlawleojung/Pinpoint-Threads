...
/**
 * Threads CDN 은 같은 비디오의 여러 화질/버퍼 URL 을 병렬로 서빙 (각 URL 완전히 다름 · dedup 불가).
 * 우리는 게시글당 비디오 1개 기준이므로 **첫 URL 만** 반환.
 * (Threads 게시글이 여러 비디오 있는 경우는 극히 드묾. 카루셀 여러 비디오 케이스가 향후 나오면 재검토)
 */
