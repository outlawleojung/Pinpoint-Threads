...
- category: 다음 중 하나만 [의류, 신발, 패션잡화, 뷰티, 생활용품, 가전, 식품, 기타]
  · **상품 종류는 이미지를 최우선으로 판정.** 원문이 외국어(중/일/말레이 등)·모호·이모지 위주면 원문에 끌려가지 말고 **이미지에 실제로 보이는 물건**으로. (예: 사진이 발목양말이면 category=신발/패션잡화·검색어 "양말"이지, 원문 이모지 🥿 에 속아 "풋패드" 로 가지 마라)
- searchKeyword: 한국어 상품 검색어 1개 (2~5단어). **가능한 한 구체적으로**:
  · 원문·이미지에 브랜드·모델명 명시되어 있으면 반드시 포함 (예: "아식스 젤카야노", "나이키 코르테즈")
  · **이미지에서 보이는 구별되는 색상·변형을 반드시 포함** (예: "오니츠카 타이거 그레이", "아디다스 삼바 블랙", "베이지 니트"). 색이 상품 정체성인 경우가 많아 색 빠지면 오매칭됨.
  · 브랜드만 있고 모델 없으면 "브랜드 + 색상 + 카테고리" (예: "아식스 그레이 러닝화")
  · 브랜드도 불명확하면 "색상 + 일반 명사" (예: "블랙 발편한 러닝화")
  · 목적: 쿠팡 검색 시 원본 이미지와 **동일 색·변형** 상품이 상위에 나올 정도의 구체성.
