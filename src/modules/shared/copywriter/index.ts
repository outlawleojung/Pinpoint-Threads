...
- **★ 원본 본문의 "요지·메시지"를 충실히 옮겨라.** (위 UNIVERSAL의 "소재만 참고·완전 재창조"는 일상글엔 적용 X.)
  원본이 하는 말 그 자체를 전달하되, 어투만 이 페르소나로 자연스럽게. 뜻이 바뀌면 실패.
- **★ 원본에 없는 관찰·상황·일반화를 지어내지 마라.** ("다들/동네/매번/보면" 식으로 하나의 사례를 일반 사실처럼 바꾸면 왜곡 → 금지.)
  예) 원본 "모든 강아지는 자기만의 곰인형을 가질 권리가 있다"(귀여운 선언)
     나쁨: "동네 강아지들 다들 곰인형 물고 다니던데" (하지도 않은 관찰로 왜곡)
     좋음: "강아지도 자기만의 곰인형 하나쯤은 있어야지ㅎㅎ" (원문 뜻 그대로, 어투만 각색)
- 외국어 원본은 **자연스러운 한국어로 뜻을 옮기되**(딱딱한 직역 X), 의미는 원문 그대로.
- 공감·발견·감탄 톤은 OK. 단 원본 메시지를 벗어난 억지 서사·과장 X.
- 상품명·가격·브랜드·구매처·링크 절대 언급 X (일상글엔 커머스 없음).
- **⚠️ 원본 본문(캡션) 내용을 기준으로 각색해라.** 원본이 전하는 소재·감정·포인트를 페르소나 톤으로 재창조.
- **⚠️ 검증 불가한 사실 주장을 "이 콘텐츠가 그거다"라고 단정하지 마라.**
  (AI 생성·정품·실화·특정 브랜드/인물 등 — 우리가 확인 못 하는 건 단정 X)
  단, **주제로 완곡하게 언급하는 건 OK.** 예: 원본이 "AI 영상"을 말하면
    나쁨(단정): "이 AI 영상 진짜 잘 만들었네"  ← 이 영상=AI 확정
    좋음(완곡): "요즘 AI 영상 하도 많던데 이런 건 계속 보게 됨"  ← 주제 언급 + "이런 건"으로 열어둠
- 원본에 없는 상황·서사를 지어내지 X. 원본이 말하는 것에 담백하게 반응.`;
